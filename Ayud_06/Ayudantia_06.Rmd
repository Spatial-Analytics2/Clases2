...
```{r educación, echo=F}
personas <- data.table(st_read("db16512a-0cbd-4d34-8d69-99e1988b064e2020328-1-n9kghv.3x70s/db16512a-0cbd-4d34-8d69-99e1988b064e2020328-1-n9kghv.3x70s.shp"))
personas <- personas[ZONA_CENSA %between% c(1,4) & CODIGO_DIS %in% c(1,2,3,8,9) & CUT == 5109,]
...
base <- merge(manzanas, personas[,.(MANZENT,TOTAL_PERS)], by.x = "MANZENT_I",by.y = "MANZENT")
```
...
```{r leaflet 5, echo=F}
pal <- colorBin("YlOrRd", domain = base$TOTAL_PERS)

leaflet(base_lf) %>% 
  addTiles() %>%
  addPolygons(fillColor = ~pal(TOTAL_PERS), weight = 2, opacity = 1, color = "black", dashArray = "3",fillOpacity = 0.5,popup = ~TOTAL_PERS) %>%
  addLegend(pal = pal, values = ~TOTAL_PERS, opacity = 1,position = "bottomright",title = "Personas")
```
...
leaflet() %>% 
  ...
  addPolygons(data = base_lf,fillColor = ~pal(TOTAL_PERS), weight = 2, opacity = 1, color = "black", dashArray = "3",fillOpacity = 0.5,popup = ~TOTAL_PERS, options = pathOptions(pane = "ames_polygon")) %>%
  addHeatmap(data = Restaurantes2,lng = ~geometry.location.lng, lat = ~geometry.location.lat, blur = 10, max = 0.001, radius = 10) %>%
  addLegend(data = base_lf, pal = pal, values = base_lf$TOTAL_PERS, opacity = 1,position = "bottomright",title = "Personas") 
```